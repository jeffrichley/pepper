...
### Task 9: Clean up repo — replace runtime config with dev config

Now that everything is in `src/pepper/`, remove the old runtime locations, replace `.claude/` with dev-oriented config, and replace `CLAUDE.md` with a dev codebase guide.

**Files:**
- Delete: `.claude/hooks/`, `.claude/scripts/`, `.claude/skills/`
- Delete: `channel/` directory (TypeScript channel server)
- Delete: `.mcp.json` (no MCP servers needed for dev)
- Rewrite: `.claude/settings.json` — dev-only settings (no hooks, no MCP)
- Rewrite: `CLAUDE.md` — dev codebase guide (not the Pepper persona)
- Modify: `.gitignore`

- [ ] **Step 1: Move skills into the package BEFORE deleting old .claude/ content**

Skills must be copied before the old directories are removed:
...
- [ ] **Step 2: Remove old runtime hook/script/skill files from .claude/**
...
- [ ] **Step 3: Remove TypeScript channel server**
...
- [ ] **Step 4: Remove runtime .mcp.json**

```bash
rm -f .mcp.json
```

- [ ] **Step 5: Replace .claude/settings.json with dev config**
...
No hooks, no MCP servers — clean dev environment. Dev agents code here without getting the Pepper persona.

- [ ] **Step 6: Replace CLAUDE.md with dev codebase guide**
...
Add these lines:
```
# Runtime config (generated by pepper init, not tracked in repo)
.mcp.json

# Runtime vault (lives at ~/.pepper/Memory/)
Memory/
config.toml
```

Note: `.claude/` is NOT gitignored — it's the dev config and should be tracked.

- [ ] **Step 8: Run all tests to make sure nothing broke**
...
```bash
git add -A
git commit -m "refactor: replace runtime config with dev environment, add developer guide"
```

---

### Task 10: Add skill installation to pepper init

Skills were moved to `src/pepper/skills/` in Task 9. Now wire them into the generator so `pepper init` copies them into the runtime's `.claude/skills/`.

**Files:**
- Modify: `src/pepper/init/generator.py`
- Test: `tests/test_init.py`

- [ ] **Step 1: Add skill installation to the generator**
...
```bash
git add src/pepper/init/generator.py tests/test_init.py
git commit -m "feat: add skill installation to pepper init generator"
```
